...
# 标签列表
@admin.route('/tag/list/<int:page>', methods=['GET'])  # 传入整型页码
@admin_login_req
@admin_auth
def tag_list(page=None):
    if page is None:
        page = 1
    page_data = Tag.query.order_by(  # 查询并按照时间排序
        Tag.addtime.desc()
...
def change_filename(filename):
    fileinfo = os.path.splitext(filename)  # 这个方法将文件名分成后缀和前缀
    filename = str(datetime.datetime.now().strftime('%Y%m%d%H%M%S')) + str(uuid.uuid4().hex) + str(fileinfo[-1])
    return filename
...
def movie_add():
    ...
    if form.validate_on_submit():
        ...
        if not os.path.exists(app.config["UP_DIR"]):
            ...
            os.chmod(app.config["UP_DIR"], stat.S_IRWXU)  # 给目录授权可读可写
...
def movie_list(page=None):
    ...
    page_data = Movie.query.join(Tag).filter(
        Tag.id == Movie.tag_id  # 关联
    ).order_by(  # 查询并按照时间排序,多表查询
        Movie.addtime.desc()
    ).paginate(page=page, per_page=10)  # 分页操作,默认10页显示一条
...
@admin_login_req
def taobao_list(page=None):
    if page is None:
        page = 1
    page_data = Taobao.query.order_by(  # 查询并按照时间排序
        Taobao.addtime.desc()
    ).paginate(page=page, per_page=10)  # 分页操作,默认10页显示一条
...
def taobao_add():
    ...
    if form.validate_on_submit():
        ...
        from app.util.get_taobao_dict import start_get_taobao
        # # 数据库查询
        # tag = Tag.query.filter_by(name=data['name']).count()
        # if tag == 1:
        #     flash("名称已经存在！", category='err')
        ...
        redirect(url_for('admin.taobao_add'))
    return render_template("admin/taobao_add.html", form=form)


# 添加标签
@admin.route('/taobao/detail/<int:id>', methods=['GET', 'POST'])
@admin_login_req
